...
import sys, json, datetime, re, time, decimal, subprocess, random, threading, os, pymongo, traceback, base64
from functools import reduce

from BoggleCVPipeline import processImage, BoggleError

PREFIX = os.environ.get('PREFIX', '') #e.g. /boggle
ROOT_DIR = os.environ.get('ROOT_DIR', '/srv')
DB_URL = os.environ.get('MONGODB_URL', 'mongodb://localhost:27017/boggle')
...
def filterUsername(username):
    ...
    return re.sub(r'[^a-zA-Z\d]', '', username)[:32]
...
def load_page(form, page=None, id=None):
    ...
    if page == "login":
        return render_template("login.html", page=page, **kwargs)

    get_param(form, kwargs, "username", filterUsername)

    if page == "pregame" and id is not None:
        return render_template("pregame.html", **kwargs)

    if page == "play" and id is not None:
        return render_template("play.html", **kwargs)

    if page == "view" and id is not None:
        return render_template("view.html", prev=prev, **kwargs)
    ...
    if page == "stats":
        ...
        # print(kwargs)
        return render_template("stats.html", prev=prev, games=games_sorted, **kwargs)
    ...
    if page == "lobby":
        return render_template("lobby.html", remove_completed_from_lobby_timeout=REMOVE_COMPLETED_FROM_LOBBY_TIMEOUT, **kwargs)
